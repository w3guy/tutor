...
class Utils {
	/**
	 * Load view pages from views directory
	 *
	 * @param string $view_path view path.
	 * @param array  $variables variables for view.
	 * @return void
	 *
	 * @throws \Exception If view file is missing.
	 */
	public function load_view( $view_path, $variables = array() ) {
		$view_path  = str_replace( '.', '/', str_replace( '.php', '', $view_path ) );
		$final_path = TUTOR_PLUGIN_DIR . "views/{$view_path}.php";

		if ( ! file_exists( $final_path ) ) {
			throw new \Exception( 'View page not found. Path:' . $final_path );
		}
