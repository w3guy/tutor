...
class QuizApi {
	...
	public function quiz_with_settings( WP_REST_Request $request ) {
		...
		return $this->api_fail( __( 'Quiz not found for given ID', 'tutor' ) );
	}
	...
	public function quiz_question_ans( WP_REST_Request $request ) {
		...
		return $this->api_fail( __( 'Question not found for given ID', 'tutor' ) );
	}
	...
	public function quiz_attempt_details( WP_REST_Request $request ) {
		...
		if ( count( $attempts ) > 0 ) {
			...
			return $this->api_data( $attempts );
		}

		return $this->api_fail( __( 'Quiz attempts not found for given ID', 'tutor' ) );
	}
