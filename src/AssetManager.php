...
class AssetManager {
	...
	public function __construct() {
		$this->asset_version = TUTOR_ASSET_VERSION;
	...
	public function common_scripts() {
		...
		// Fonts.
		wp_enqueue_style( 'tutor-inter-fonts', 'https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap', array(), $this->asset_version );
		wp_enqueue_style( 'tutor-icon', tutor()->url . 'assets/css/tutor-icon.min.css', array(), $this->asset_version );

		// Common css library.
		wp_enqueue_style( 'tutor', tutor()->url . 'assets/css/tutor.min.css', array(), $this->asset_version );

		// Load course builder resources.
		if ( tutor_utils()->get_course_builder_screen() ) {
			wp_enqueue_script( 'tutor-course-builder', tutor()->url . 'assets/js/tutor-course-builder.min.js', array( 'jquery', 'wp-i18n' ), $this->asset_version, true );
			wp_enqueue_style( 'tutor-course-builder-css', tutor()->url . 'assets/css/tutor-course-builder.min.css', array(), $this->asset_version );
		}
		 ...
		 * @since v2.0.0
		 */
		wp_enqueue_script( 'tutor-script', tutor()->url . 'assets/js/tutor.min.js', array( 'jquery', 'wp-i18n' ), $this->asset_version, true );
	}

	/**
	 * Load text domain script
	 *
	 * @return void
	 *
	 * @since 1.9.0
	...
	public function modify_localize_data( $localize_data ) {
		...
		if ( is_admin() ) {
			if ( ! empty( Input::get( 'taxonomy' ) ) && ( 'course-category' === Input::get( 'taxonomy' ) || 'course-tag' === Input::get( 'taxonomy' ) ) ) {
	 ...
	 *
	 * @param string $gen Generator Generator.
	...
	public function tutor_generator_tag( $gen, $type ) {
		switch ( $type ) {
			case 'html':
				$gen .= "\n" . '<meta name="generator" content="TutorLMS ' . $this->asset_version . '">';
				break;
			case 'xhtml':
				$gen .= "\n" . '<meta name="generator" content="TutorLMS ' . $this->asset_version . '" />';
				break;
	...
	/**
	 * Load admin assets like css, js files
	 *
	 * @return void
	 */
	public function load_admin_assets() {
		wp_enqueue_style( 'tutor-select2', tutor()->url . 'assets/packages/select2/select2.min.css', array(), $this->asset_version );
		wp_enqueue_style( 'tutor-admin', tutor()->url . 'assets/css/tutor-admin.min.css', array(), $this->asset_version );
		...
		wp_enqueue_script( 'tutor-select2', tutor()->url . 'assets/packages/select2/select2.full.min.js', array( 'jquery' ), $this->asset_version, true );
		wp_enqueue_script( 'tutor-admin', tutor()->url . 'assets/js/tutor-admin.min.js', array( 'jquery', 'wp-color-picker', 'wp-i18n' ), $this->asset_version, true );
	}

	/**
	 * Load site assets like css, js files
	 *
	 * @return void
	 */
	public function load_site_assets() {
		...
		if ( is_single_course( true ) ) {
			wp_enqueue_style( 'tutor-plyr', tutor()->url . 'assets/packages/plyr/plyr.css', array(), $this->asset_version );
			wp_enqueue_script( 'tutor-plyr', tutor()->url . 'assets/packages/plyr/plyr.polyfilled.min.js', array( 'jquery' ), $this->asset_version, true );
		}

		// Social Share.
		wp_enqueue_script( 'tutor-social-share', tutor()->url . 'assets/packages/SocialShare/SocialShare.min.js', array( 'jquery' ), $this->asset_version, true );
		...
		if ( ! empty( $wp_query->query_vars['tutor_dashboard_page'] ) ) {
			...
			if ( 'earning' === $wp_query->query_vars['tutor_dashboard_page'] ) {
				wp_enqueue_script( 'tutor-front-chart-js', tutor()->url . 'assets/js/lib/Chart.bundle.min.js', array(), $this->asset_version, true );
				wp_enqueue_script( 'jquery-ui-datepicker' );
			}
		}
		 ...
		 * @since 1.9.0
		 */
		wp_enqueue_style( 'tutor-frontend', tutor()->url . 'assets/css/tutor-front.min.css', array(), $this->asset_version );
		wp_enqueue_script( 'tutor-frontend', tutor()->url . 'assets/js/tutor-front.min.js', array( 'jquery', 'wp-i18n' ), $this->asset_version, true );
		...
		if ( tutor_utils()->is_tutor_frontend_dashboard() ) {
			wp_enqueue_style( 'tutor-frontend-dashboard-css', tutor()->url . 'assets/css/tutor-frontend-dashboard.min.css', array(), $this->asset_version );
		}
	...
	private function get_default_localized_data() {
		...
		return array(
			...
			'is_tutor_course_edit'         => Input::has( 'action' ) && 'edit' === Input::get( 'action' ) && tutor()->course_post_type === get_post_type( get_the_ID() ) ? true : false,
	...
	public function tutor_tinymce_translate() {
		$locales['tutor_button'] = tutor()->path . 'src/Includes/tinymce_translate.php';
		return $locales;
	}
	...
	public function add_identifier_class_to_body( $classes ) {
		...
		if ( is_admin() ) {
			...
			if ( 0 === $index || $index > 0 ) {
				...
				if ( Input::get( 'page' ) === 'tutor_settings' ) {
					$to_add[] = 'tutor-screen-backend-settings ';
				}
				if ( Input::has( 'page' ) ) {
					$to_add[] = 'tutor-backend-' . Input::get( 'page' );
				}
			}
		}
