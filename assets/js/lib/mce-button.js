jQuery(document).ready(function ($) {
    ...
    tinymce.PluginManager.add('tutor_button', function (editor, url) {
        ...
        let iframeObj = {
            text: editor.getLang('tutor_button.iframe'),
            onclick: function () {
                editor.windowManager.open({
                    title: editor.getLang('tutor_button.tutor_iframe'),
                    body: [
                        {
                            type: 'textbox',
                            name: 'url',
                            label: editor.getLang('tutor_button.url'),
                            value: ''
                        },
                        {
                            type: 'textbox',
                            name: 'width',
                            label: editor.getLang('tutor_button.width'),
                            value: '400'
                        },
                        {
                            type: 'textbox',
                            name: 'height',
                            label: editor.getLang('tutor_button.height'),
                            value: '400',
                        }
                    ],
                    onsubmit: function (e) {
                        editor.insertContent('[tutor_iframe url="' + e.data.url + '" width="' + e.data.width + '" height="' + e.data.height + '"]');
                    }
                });
        ...
        if (_tutorobject.has_pro === '1') {
            menuArray.unshift(iframeObj)
        }
